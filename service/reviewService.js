...
const createReview = async (reviewData) => {
  try {
    if (!reviewData.product && !reviewData.store) {
      throw new Error('A review must be associated with either a product or a store.');
    }
...
const getReviewsForStore = async (storeId) => {
  try {
    const reviews = await Review.find({ store: storeId }).populate('user', 'name email');
    return reviews;
  } catch (error) {
    throw new Error(error.message);
  }
...
const calculateAverageRating = async (filters) => {
  try {
